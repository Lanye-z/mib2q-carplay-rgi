...
BUILD_ID = "2026-08-06-main-release"
    ...
    while i < cp_count:
        ...
        if tag == 1:
            ...
            try:
                ...
                text = ""
            if text.startswith("Build: 2026-") and text != "Build: " + new_build_id:
                raw = ("Build: " + new_build_id).encode("utf-8")
                changed = True
            elif text.startswith("2026-") and text != new_build_id:
...
def main() -> None:
    if len(sys.argv) != 1:
        raise SystemExit("usage: build_release.py")
    build_id = BUILD_ID
    base_jar = ROOT / "build_assets/carplay_hook_base.jar"
    ...
    sums = [digest + "  carplay_hook.jar"]
    renderer = dist / "maneuver_render"
    if renderer.exists():
        renderer_digest = hashlib.sha256(renderer.read_bytes()).hexdigest()
        sums.append(renderer_digest + "  maneuver_render")
